...
def checkForUpdate():
    ...
    v = requests.get("https://raw.githubusercontent.com/electricboy6/WinRepairTool/main/program.version", allow_redirects=True)
    currentV = open("source.version").read()
    if not v == currentV:
        print("Found update - Downloading...")
        r = requests.get("https://api.github.com/repos/electricboy6/WinRepairTool/releases/latest")
        version = r.json()["name"]
        r = requests.get("https://github.com/electricboy6/WinRepairTool/releases/tag/" + version)
